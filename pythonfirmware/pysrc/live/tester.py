...
cs = Pin(pinout.DISP_CS)
dc = Pin(pinout.DISP_DC)
rst = Pin(pinout.DISP_RST)
busy = Pin(pinout.DISP_BUSY)
...
def drawText(s):
    fb.fill_rect(0, 0, w, 40, white)
    fb.text(s, 30, 0, black)
    e.set_frame_memory(buf, x, y, w, h)
    e.display_frame()


def drawPartialText(s):
    e.reset()
    fb.fill_rect(0, 0, w, 40, white)
    fb.text(s, 30, 0, black)
    e.set_frame_memory(buf, x, y, w, h)
    e.set_lut(epaper.EPD.LUT_PARTIAL_UPDATE)
    e.display_frame()
    e.sleep()
